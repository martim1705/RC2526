...
int main(int argc, char *argv[])
{
    ...
    while (STOP == FALSE)
    {
        ...
        if (nBytesBuf == 2 && byte != 0x03) received = 0;
        if (nBytesBuf == 3 && byte != 0x03) received = 0;
        if (nBytesBuf == 4 && byte != 0x00) received = 0;
        ...
        if (nBytesBuf == 5 && byte == FLAG)
        {
            ...
            if (received) {
                ...
                int bytes2write = writeBytesSerialPort(buf, BUF_SIZE);
            }
            STOP = TRUE;
        }
    }
